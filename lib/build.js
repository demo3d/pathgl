...
function build(_, file) {
  ...
  .forEach(function (abc) {
    var file = template(marked(fs.readFileSync('documentation/' + abc).toString()))
  ...
  .forEach(function (abc) {
    var file = template('<script src="' + abc + '"></script>', '<a href="' + abc + '" target="_wow">View Source</a>', abc.replace(/\..*/, ''))
...
function template (str, src, page) {
