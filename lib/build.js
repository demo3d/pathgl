...
function build(_, file) {
  ...
  .forEach(function (name) {
    var annotation = '<code>' + fs.readFileSync('examples/md/' + name) + '</code>' +
      '<code>' + fs.readFileSync('examples/js/' + name) + '</code>'

    var file = template('<script>\n'+ fs.readFileSync('examples/js/' + name) + '\n</script>',
                        annotation,
                        name.replace(/\..*/, ''),
                        'examples')
